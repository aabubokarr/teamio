...
function RouteComponent() {
  ...
  return (
    <div className="space-y-4 mb-10 justify-self-center">
      {/* Header Section */}
      <div className="flex flex-col gap-2">
         {/* Row 1: Date and Update Info */}
         <div className="flex items-center justify-between">
           <span className="text-sm font-medium text-[#1A1D1F]">Date: 16/03/2023</span>
           <div className="flex items-center gap-2 text-sm text-[#6F767E]">
             <div className="size-1.5 rounded-full bg-[#FF3B30]" />
             <span>Update 13 min ago</span>
           </div>
         </div>

         {/* Row 2: Title and Team */}
         <div className="flex items-center justify-between">
            <div className="flex items-center gap-3 md:gap-4">
              <div className="size-10 md:size-12 rounded-[12px] bg-[#FFE600]" />
              <h1 className="text-[24px] md:text-[32px] font-bold text-[#1A1D1F] tracking-tight">Design System</h1>
            </div>
            ...
            <div className="flex items-center">
              ...
              <button
                ...
                className="size-8 md:size-10 rounded-full bg-black text-white flex items-center justify-center shrink-0 -ml-2 md:-ml-3 border-2 border-white hover:bg-gray-800 transition shadow-sm"
              >
         ...
         {/* Row 3: Toolbar (Search | Toggle | Dropdowns) */}
         <div className="flex flex-col xl:flex-row items-stretch xl:items-center justify-between gap-4 xl:gap-0 mt-2">
            
            {/* Left: Search */}
            <div className="relative flex-1 xl:flex-none xl:w-[320px]">
                <IconSearch className="pointer-events-none absolute left-4 top-1/2 size-5 -translate-y-1/2 text-black" stroke={2} />
                <input
                    type="search"
                    placeholder="Search Task"
                    className="w-full h-12 rounded-[14px] bg-[#F4F4F4] pl-12 pr-4 text-[15px] font-medium text-[#1A1D1F] outline-none placeholder:text-[#1A1D1F]"
                />
            </div>

            {/* Center: View Toggle */}
            <div className="flex justify-center">
                <div className="bg-[#F4F4F4] p-1 rounded-[14px] flex items-center gap-1">
                    <button
                       onClick={() => setViewMode("board")}
                       className={`flex items-center gap-2 px-6 py-2.5 rounded-[12px] text-[15px] font-semibold transition-all ${
                           viewMode === "board" ? "bg-white shadow-[0_2px_8px_0_rgba(0,0,0,0.08)] text-[#1A1D1F]" : "text-[#6F767E] hover:bg-gray-200"
                       }`}
                    >
                       <svg className="size-4" viewBox="0 0 24 24" fill={viewMode === 'board' ? 'currentColor' : '#6F767E'}><path d="M4 4h6v16H4V4zm10 0h6v16h-6V4z"/></svg>
                       Board
                    </button>
                    <button
                       onClick={() => setViewMode("list")}
                       className={`flex items-center gap-2 px-6 py-2.5 rounded-[12px] text-[15px] font-semibold transition-all ${
                           viewMode === "list" ? "bg-white shadow-[0_2px_8px_0_rgba(0,0,0,0.08)] text-[#1A1D1F]" : "text-[#6F767E] hover:bg-gray-200"
                       }`}
                    >
                       <svg className="size-4" viewBox="0 0 24 24" fill={viewMode === 'list' ? 'currentColor' : '#6F767E'}><path d="M4 6h16v2H4V6zm0 5h16v2H4v-2zm0 5h16v2H4v-2z"/></svg> 
                       List
                    </button>
                </div>
            </div>
            ...
            {/* Right: Dropdowns */}
            <div className="grid grid-cols-2 lg:flex items-center gap-3 relative">
                 {/* To-do Dropdown */}
                 <div className="relative">
                     <button 
                        onClick={() => setActiveDropdown(activeDropdown === 'todo' ? null : 'todo')}
                        className="w-full lg:w-auto h-12 px-5 bg-black text-white rounded-[14px] flex items-center justify-between lg:justify-start gap-3 text-[15px] font-bold hover:bg-gray-800 transition"
                     >
                        <div className="flex items-center gap-2">
                            <svg className="size-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M3 6h18M7 12h10m-8 6h6"/></svg>
                            To-do
                        </div>
                        <IconChevronDown className={`size-4 opacity-70 transition-transform ${activeDropdown === 'todo' ? 'rotate-180' : ''}`} stroke={3} />
                     </button>
                 ...
                 <div className="relative">
                     ...
                     >
                        <div className="flex items-center gap-2">
                             <svg className="size-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>
                             Today
                        </div>
                        <IconChevronDown className={`size-4 opacity-70 transition-transform ${activeDropdown === 'today' ? 'rotate-180' : ''}`} stroke={3} />
                     </button>
      ...
      {viewMode === "board" && (
        <div className="-mx-4 md:mx-0">
          <div className="overflow-x-auto pb-4 px-4 md:px-0">
            <div className="flex lg:grid gap-4 md:gap-6 lg:grid-cols-3 min-w-max lg:min-w-0">
            {columns.map((column) => (
              <div
                key={column.id}

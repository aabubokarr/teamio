...
export function EditScheduleModal({
  ...
  const [selectedHosts, setSelectedHosts] = useState<string[]>(scheduleData.hosts || []);
  const [selectedColor, setSelectedColor] = useState<"green" | "purple" | "blue">(
    scheduleData.color || "green"
  );
  const [formData, setFormData] = useState<ScheduleData>(scheduleData);

  useEffect(() => {
    if (isOpen) {
      setFormData(scheduleData);
      setSelectedHosts(scheduleData.hosts || []);
      setSelectedColor(scheduleData.color || "green");
    }
  }, [scheduleData, isOpen]);
    ...
    <DialogTrigger isOpen={isOpen} onOpenChange={onOpenChange}>
      ...
      <ModalOverlay className="grid center-grid items-center [--container:min(90%,--spacing(120))]">
        <Modal className="bg-white rounded-2xl shadow-lg max-w-2xl w-full max-h-[90vh] flex flex-col overflow-hidden">
          <Dialog aria-label="Edit Schedule" className="focus-visible:outline-none flex flex-col h-full max-h-[90vh]">
            {({ close }) => (
              <div className="flex flex-col h-full max-h-[90vh]">
                ...
                <div className="flex-1 overflow-y-auto px-6 py-6 min-h-0">
                  <form className="space-y-5" onSubmit={handleSubmit}>
                    ...
                    {/* Meeting Link Field */}
                    <div className="flex flex-col gap-2">
                      <label htmlFor="edit-meetingLink" className="text-sm font-medium text-gray-700">
                        Meeting Link
                      </label>
                      <div className="relative">
                    ...
                    {/* Hosted by Section */}
                    <div className="flex flex-col gap-2">
                      <label htmlFor="edit-hostedBy" className="text-sm font-medium text-gray-700">
                        Hosted by
                      </label>
                      <div className="relative">
